...
// 4. 드래그 드랍
{
 ...
 dropZone.ondrop = function(e){
    e.preventDefault();
    console.log(e.dataTransfer.files.length);//파일 수 확인 가능
    dropZone.classList.remove("invalid");
    dropZone.classList.remove("valid");
    ...
    // 3. 목록을 키보드로 관리
    {
        ...
        let delButton = s1.querySelector(".btn-del");
        s1.focus();
        s1.onkeydown = function(e){
            // if (e.keyCode == ) {
        ...
        delButton.onclick = function () {
            ...
            let selectedButtons = [...ul.querySelectorAll("input:checked")].map(a=>a.parentElement);
